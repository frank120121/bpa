...
async def populate_bank_account_cache(conn, bank):
    try:
...
async def get_payment_details(conn, order_no, buyer_name, oxxo_used=False):
    try:
        ...
        if not bank_accounts[buyer_bank]:
            bank_accounts[buyer_bank] = await populate_bank_account_cache(conn, buyer_bank)
        ...
        if not oxxo_used and buyer_bank not in ['oxxo', 'banregio']:
            bank_accounts['nvio'] = await populate_bank_account_cache(conn, None, None, 'nvio')
...
async def initialize_bank_account_cache(conn):
    for bank in ['nvio', 'oxxo', 'santander']:
        bank_accounts[bank] = await populate_bank_account_cache(conn, bank)
...
async def main():
    ...
    if conn is not None:
        try:
            buyer_bank = 'oxxo'
            suitable_accounts = await populate_bank_account_cache(conn, buyer_bank)
